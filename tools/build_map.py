...
---
type: 用語            # 用語 / 人物 / 実験 / 症例 / 書物 / 映像 / 遊戯 / 理論
...
# 生物 = 本に登場した動物・植物（2026-09-12）。学名・分類・図鑑（動物図鑑の番号）の欄を持てる。
#   「この本に出てくる動物」「図鑑で出会いたい動物」として引くための型。
# MEDIA-2026-09-23 映像＝動画（映NN）・遊戯＝ゲーム（遊NN）。それまで書物で代用していた。
#   ⚠index.html の TYPE_SHAPE・TYPE_COLOR・ORDER・SRC_GROUPS と必ず揃える
...
def main():
    ...
    for fn in sorted(os.listdir(NODES_DIR)):
        ...
        fields = {}
        for k in (PAPER_FIELDS + BIO_FIELDS):
            if meta.get(k):
                fields[k] = meta[k]
